...
namespace StartUpApi.Data.DbScript
{
    /// <summary>
    /// Extension that allow executing of custom scripts on the DbContext
    /// </summary>
    internal class DbScriptProcessor : BaseProcessor, IDbScriptProcessor
    {
    ...
    internal abstract class BaseProcessor
    {
        ...
        {
            ScriptResult responseBase = new ScriptResult();
            try
            {
                executeMethod();
        ...
        {
            ScriptResult<T> responseBase = new ScriptResult<T>();
            try
            {
                responseBase.Data = executeMethod();
                responseBase.Status = true;
            }
            catch (Exception ex)
            {
